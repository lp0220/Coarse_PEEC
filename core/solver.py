...
class PEECSolver:
    ...
    def compute_self_l(self, sub_grids, axis, dims):
        """计算支路的自感（基于细分子网格的积分和）"""
        total_a1 = 0.0
        for g_i in sub_grids:
            for g_j in sub_grids:
                # 基础 Mt 部分
                mt = self.calculate_mt_value(g_i, g_j)
                a1_p1 = U0 * mt

                # A1 Part 2 (高斯积分简化版，根据需要可扩展 A2, A3)
                # 此处保留原逻辑中对 A1_p1 的主要贡献计算
                total_a1 += a1_p1
        return total_a1
            ...
            for yi, wyi in zip(yi_p, yi_w):
                ...
                for xm, wxm in zip(xm_p, xm_w):
                    for ym, wym in zip(ym_p, ym_w):
                        ...
                        # 仅针对 Part 2 和 A3 的 1/R 奇异性进行保护
                        if R_val < self.math_eps:
                            # 如果距离极小（重合点），强制设为微小值，防止除零
                            R_hat = np.zeros(3)
                            R_eff = self.math_eps
